 ...
 * @returns 
 */
export function getCycle(year: number, month: number) {
...
export function fullMoonDays(year: number): string[] {
  const tz: TimeZones = "GMT";
  const ct: CalendarTypes = "Gregorian";
  const phases = new Array(12).fill(0).map((_, i) => calMoonPhases(year, i));
  return phases.map((p) => jd2DateTime(p.fullMoon, tz, ct).string);
}
function utcTime(): {
  year: number;
  month: number;
